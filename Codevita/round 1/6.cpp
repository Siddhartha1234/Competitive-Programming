  ...
  rep(a,0,15){
     rep(b,0,15){
       rep(ln,1,7){
         rep(shift,0,ln){
           ll curr = 0, prod = 1;
           rep(bb,1,ln-shift) curr += (prod*b), prod *= 16;
           rep(aa,1,shift) curr += (prod*a), prod *= 16;
           //db(curr);
           if(curr > n && curr%n == 0) ans = min(ans,curr);
         }
       }
     }
  }
  if(ans == LLONG_MAX) cout<<"NOTFOUND"<<endl;
  else {
    stringstream ss;
    ss<<hex<<ans;
    string res = ss.str();  
    rep(i,0,len(res)-1) if(isalpha(res[i])) res[i] = toupper(res[i]);
    cout<<res<<endl;
  }
}
/* main function */
