...
{ 
   cin>>n>>m;
   ll bb,g;
   cin>>bb; rep(i,1,bb) { ll bbb; cin>>bbb; happyb[bbb] = 1;}
   cin>>g; rep(i,1,g) { ll cc; cin>>cc; happyg[cc] = 1;}
   rep(i,0,maxn) if(happyb[i%n] || happyg[i%m])happyb[i%n] = 1, happyg[i%m] = 1;
   int happy = 1;
   rep(i,0,n-1) if(!happyb[i]) happy = 0;
   rep(i,0,m-1) if(!happyg[i]) happy = 0;
   if(happy) cout<<"Yes";
   else cout<<"No";
