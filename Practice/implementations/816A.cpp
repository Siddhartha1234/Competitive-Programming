  ...
  */
  ...
  ll T;
  const ll maxn = 2e5;
  string nxt(string curr)
  {
      string hh,mm;
      ll ind = 0;
      while(ind < len(curr) && curr[ind] != ':') hh += curr[ind] ,ind++;
      ind++;
      while(ind < len(curr)) mm += curr[ind], ind++;
      //db(hh) db(mm)endln;
      int minute = stoi(mm);
      int hour = stoi(hh);
      ...
      mm = to_string(minute);
      hh = to_string(hour);
      if(len(hh) == 1) res += '0' + hh;
      else res += hh;
      res += ':';
      if(len(mm) == 1) res += '0' + mm;
      else res += mm;
      return res; 
  }
